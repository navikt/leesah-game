    ...
    fun `publish a quiz message`() {
        ...
        val quizmasterServer =
            RapidServer(
                ...
                records.forEach {
                    if(it.value().contains("team-registration")) publish("OK")
                }
            }
        ...
        Awaitility.await("wait until assessment received")
            ...
            .until {
                records(testTopic)?.any {
                    it.value().contains("TEST MESSAGE")
                } ?: false
            }
...
fun testKtor(): ApplicationEngine = embeddedServer(CIO, applicationEngineEnvironment {
    connector {
        port = 64532
    }
    module {
