...
export type Backend = {
  ...
  acceptAnswer(answerId: string, teamName: string): Promise<CategoriesDto>;
  deleteTeam(team: string): Promise<string>;
};
...
export const restBackend = (development: boolean): Backend => {
  ...
  return {
    ...
    acceptAnswer(category: string, teamName: string): Promise<CategoriesDto> {
    ...
    deleteTeam(teamName: string) {
      return fetch(`${baseUrl}/slett/${teamName}`, {
        method: "put",
        headers: {
          Accept: "application/json",
        },
...
export const errorWhenBadResponse = async (
  response: Response,
): Promise<Response> => {
  if (!response.ok) {
